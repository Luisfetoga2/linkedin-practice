...
import { clueLeaksAnswer, foldAccents } from '../parse';
import { ENTRIES, OWN_CHUNKS, SHARED_CHUNKS } from './index';
...
describe('Spanish Mini vocabulary', () => {
  ...
  it('is dense enough for 5×5 grids with few black squares', () => {
    ...
    expect(count(3)).toBeGreaterThanOrEqual(200);
    expect(count(4)).toBeGreaterThanOrEqual(1000);
    expect(count(5)).toBeGreaterThanOrEqual(2500);
  });
